...
public class OneHitKill implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label,
		...
		if (cmd.getName().equalsIgnoreCase("onehitkill")) {
			...
			Player p = (Player) cs;
			Boolean ohk = PConfManager.getPValBoolean((OfflinePlayer) p, "ohk");
			if (ohk.equals(null) || !ohk) {
				PConfManager.setPValBoolean((OfflinePlayer) p, true, "ohk");
				p.sendMessage(ChatColor.BLUE
						+ "You have enabled onehitkill for yourself.");
				return true;
			}
			if (ohk) {
				PConfManager.setPValBoolean((OfflinePlayer) p, false, "ohk");
				p.sendMessage(ChatColor.BLUE
